...
class Record:
    ...
    def output_all_salary(self, new_salaries: list[Optional[float]]|float) -> list[float]:
        newlist: list[float] = []

        if isinstance(new_salaries, float):
            newlist.append(new_salaries)
            newlist.extend(self.salaries)
            return newlist
        for item in new_salaries:
