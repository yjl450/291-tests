...
class Record:
    ...
    # or it can be a list of values or None (such as from data se query where some months' data are missing)
    ...
    def output_all_salary(self, new_salaries: float|list[float|None]) -> list[float]:
        ...
        if isinstance(new_salaries,list):
            for salary in new_salaries:
                # if (salary != None): <- original, bug not solved
                if salary is not None: # correct
